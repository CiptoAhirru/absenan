...
@section('container')
    ...
    <div class="col-lg-5">
        <form method="POST" action="/karyawan/{{ $karyawan->slug }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="mb-3">
                ...
                <input type="text" class="form-control" id="name" name="name" placeholder="new Karyawan..."
                    value="{{ old('name', $karyawan->name) }}" required autofocus>
            </div>
            <div class="mb-3">
                <label for="slug" class="form-label">Nickname</label>
                <input type="text" class="form-control" value="{{ old('slug', $karyawan->slug) }}" id="slug"
                    name="slug" placeholder="" required autofocus>
            </div>
            <div class="mb-3">
                <label for="gaji" class="form-label">Gaji Harian</label>
                <input type="number" class="form-control" id="gaji" name="gaji"
                    value="{{ old('gaji', $karyawan->gaji) }}" required autofocus>
            </div>
